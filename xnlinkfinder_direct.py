...
def showBanner():
    ...
    write(colored(r"                ' by @Xnl-h4ck3r '              v" + "TEST"))
    write("")
    showVersion()
...
def processUrl(url):
    global burpFile, zapFile, caidoFile, totalRequests, skippedRequests, failedRequests, userAgent, requestHeaders, tooManyRequests, tooManyForbidden, tooManyTimeouts, tooManyConnectionErrors, stopProgram, waymoreMode, failedPrefixLinks, currentDepth
            ...
            if stopProgram is None:
                try:
                    ...
                    if not args.scope_prefix_keep_failed and prefixed and resp.status_code == 404:
                        ...
                        # Immediately write the links to the output file and stdout if `-do` is used
...
def processEachInput(input):
    global burpFile, zapFile, caidoFile, stdFile, inputFile, urlPassed, dirPassed, stdinMultiple, linksFound, linksVisited, totalRequests, skippedRequests, failedRequests, paramsFound, waymoreMode, stopProgram, contentTypesProcessed, oosLinksFound, lstPathWords, wordsFound
    ...
    try:
        ...
        if args.scope_filter is None and not waymoreMode and (urlPassed or stdFile or stdinFile):
            writerr(
                colored(
                    "You need to provide a Scope Filter with the -sf / --scope-filter argument. This was optional in previous versions but is now mandatory if input is a domain/URL (or file of domains/URLs) to prevent crawling sites that are not in scope, and also prevent misleading results.",
                    "red",
                )
            )
            sys.exit()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="xnlLinkFinder (v" + "TEST" + ") - by @Xnl-h4ck3r"
    )
    parser.add_argument(
        "-i",
        "--input",
    ...
    parser.add_argument(
        '-do', '--direct-output', action='store_true',
        help="If set, each found link/parameter is immediately written to the output file rather than aggregated and output at the end (useful for large input files)."
    )
...
if __name__ == '__main__':
    try:
        ...
        processOutput()
        writerr("Gracefully exiting... Partial result saved if any")
        sys.exit(0)
